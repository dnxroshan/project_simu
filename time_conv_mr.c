...
int Play(double dbK, double dbb);
...
void main()
{
	...
	int t,re;
	FILE *fp;
	char fname[100],oname[100];

   	init_genrand64(time(0));

	for(re = 0;re<100;re++)
	{
		sprintf(oname,"/home/dnroshan/Project/output/p-t/MR/ring_1024_2/time/ring_1024_time(%d).dat",re);
		fp = fopen(oname,"w");
		for(p=0;p<=1;p+=0.01)
		{
			sprintf(fname,"/home/dnroshan/Project/networks/ring_1024_2_hr/ring_%d_%d_%3.2f.dat",1024,2,p);
	        	printf("%s %f \n ",fname,p);
			LoadAgents(fname);
			t=Play(K_NOI,B);
			fprintf(fp,"\n%f %d", p, t);
